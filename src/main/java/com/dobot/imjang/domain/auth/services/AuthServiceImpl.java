...
public class AuthServiceImpl implements AuthService {
  ...
  public String login(LoginRequestDto loginRequestDto) throws Exception {
    Optional<Member> optional = this.memberRepository.findByEmail(loginRequestDto.getEmail());
    if (optional.isPresent()) {
      Member member = optional.get();
      if (this.passwordEncoder.matches(loginRequestDto.getPassword(), member.getPassword())) {
